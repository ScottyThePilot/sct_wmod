...
if (_missingComponentsRequired isNotEqualTo []) exitWith {
  ...
  (format [
    localize PLSTRING(notify_missing_components),
    (_missingComponentsRequired apply { _x select 1 }) call PFUNC(textList)
  ]) call CBA_fnc_notify;
};
...
if (_missingToolsRequired isNotEqualTo []) exitWith {
  ...
  (format [
    localize PLSTRING(notify_missing_tools),
    (_missingToolsRequired apply { _x select 1 }) call PFUNC(textList)
  ]) call CBA_fnc_notify;
};
